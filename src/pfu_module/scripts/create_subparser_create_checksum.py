...
def create_subparser_create_checksum(subparsers):
    ...
    # pylint: disable=invalid-name
    help_create = ""
    help_create += "This command will create (missing) checksums. " + \
        "Command line parameters can be shortened, as far as they are unique."
    myposthelp = "Example:\n\n"
    ...
    parser_create.add_argument(
        ...
        required=True,
        dest='directories',
        help='Create hashes for this directory tree or these directory ' +
        'trees. Symbolic links in given directories are ignored.',
        metavar='dir')
    parser_create.add_argument(
        '-algorithm',
        nargs=1,
        ...
        required=False,
        dest='algorithm',
        help='Set the algorithm used to calculate the hashes. ' +
        'Possible values are: md5, sha256, sha512. default: sha512',
        metavar='a')
    parser_create.add_argument(
        '-coding',
        nargs=1,
        default=['base64'],
        choices=['hex', 'base16', 'base32',
                 'base64', 'Base16', 'Base32', 'Base64'],
        type=str,
        required=False,
        dest='coding',
        help='Set the coding format (RFC 3548) of the hash output. ' +
        'Possible values are: ' +
        'hex (alias for base16), base16, base32, base64. default: base64',
        metavar='c')
    parser_create.add_argument(
        '-store',
        nargs=1,
        ...
        required=False,
        dest='store',
        help='Set the file(s) to store the hashes. ' +
        'Set to \"dir\" means store the hashes in a file for every ' +
        'directory. Set to \"single\" means store the hashes in a ' +
        'single file (for every given directory). Set to \"many\" ' +
        'means store the hashes in a file for every data file. default: dir',
        metavar='p')
    parser_create.add_argument(
        '-ignore',
        nargs="+",
        ...
        '(interpreted as hash files). default: .md5 .sha256 .sha512',
        metavar='ext')
    parser_create.add_argument(
        '-buf_size',
        nargs=1,
    ...
    parser_create.add_argument(
        ...
        required=False,
        dest='chunk_size',
        help='Set the chunk size. For every n Bytes an own checksum is ' +
        'calculated. If you want no checksums for chunks you can set ' +
        'this value to "inf". default (12 MB): 12582912',
        metavar='n')
    parser_create.add_argument(
        '-create_only_missing',
        nargs=1,
        ...
        help='If set to 1 only missing checksums are created. ' +
        'A checksum is missing, if the expected file from store is ' +
        'not available. If set to 0 hash files are overwritten if exists. ' +
        'default: 1',
        metavar='n')
    parser_create.add_argument(
        '-hash_file_prefix',
