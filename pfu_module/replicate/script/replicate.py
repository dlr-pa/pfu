...
def replicate(args):
    ...
    :Date: 2015-10-05, 2017-02-14 (last change).
    :License: GNU GENERAL PUBLIC LICENSE, Version 3, 29 June 2007.
    ...
    if not isinstance(args.limit_number_of_processes_to_distinations,int):
        args.limit_number_of_processes_to_distinations = args.limit_number_of_processes_to_distinations[0]
    ...
    args.checksum_log_file_name = re.sub("%date",today.strftime("%Y-%m-%dT%H%M%S"),args.checksum_log_file_name)
    args.checksum_status_file_name = re.sub("%date",today.strftime("%Y-%m-%dT%H%M%S"),args.checksum_status_file_name)
    # logging
    ...
    if len(args.destination) <= 1:
        ...
        log.info(" %d destinations (absolut paths): %s" % (len(args.destination),dests))
    ...
    log.info(" parameter for %s to create checksums: %s" % (args.checksum_program,args.checksum_create_parameter))
    log.info(" parameter for %s to check: %s" % (args.checksum_program,args.checksum_check_parameter))
    ...
    for dest in args.destination:
        ...
        if (platform.system() == "Windows") and (args.copy_program1 == "rsync"):
            ...
            drive_letter = re.findall('([a-zA-Z]{1}):[\\\/]{1}',source)
            if len(drive_letter) == 1:
                source = re.sub("[a-zA-Z]{1}:[\\\/]{1}",
                                ("/cygdrive/%s/" % drive_letter[0]),
                                source)
            drive_letter = re.findall('([a-zA-Z]{1}):[\\\/]{1}',destination)
            if len(drive_letter) == 1:
                destination = re.sub("[a-zA-Z]{1}:[\\\/]{1}",
                                     ("/cygdrive/%s/" % drive_letter[0]),
                                     destination)
    ...
    for (dirpath, dirnames, filenames) in os.walk(args.source):
        if ((not os.path.exists(os.path.join(dirpath, args.checksum_file_name))) or
            ...
            stat = "WARNING: '%s' already exists in '%s', but missing checksums will be created" % (args.checksum_file_name,dirpath)
            log.info(stat)
            ...
            stat = "WARNING: '%s' already exists in '%s'" % (args.checksum_file_name,dirpath)
            log.info(stat)
            warning_creating_checksums += stat + "\n"
    ...
    while ((cri < len(commands_copy1)) or
        ...
        if cri < len(commands_copy1):
            runs['processes'][i] = subprocess.Popen(commands_copy1[cri],bufsize=-1,shell=True)
            runs['cmd'][i] = commands_copy1[cri]
            log.info("started '%s'" % commands_copy1[cri])
            cri += 1
        elif ccc < len(commands_create_checksums):
            runs['processes'][i] = subprocess.Popen(commands_create_checksums[ccc],bufsize=-1,shell=True)
    ...
    log.info("all processes started for step 1/3 (copy with %s + create checksum with %s)" % (args.copy_program1,args.checksum_program))
    ...
    while cri < len(commands_copy2):
        ...
        if cri < len(commands_copy2):
            runs['processes'][i] = subprocess.Popen(commands_copy2[cri],bufsize=-1,shell=True)
            runs['cmd'][i] = commands_copy2[cri]
            log.info("started '%s'" % commands_copy2[cri])
            cri += 1
    # wait until all processes are ready
    log.info("all processes started for step 2/3 (copy with %s)" % args.copy_program2)
    for i in range(args.number_of_processes):
        if not (runs['processes'][i] is None):
            runs['processes'][i].wait()
            stat = "process '%s' terminated with %d" % (runs['cmd'][i],runs['processes'][i].returncode)
